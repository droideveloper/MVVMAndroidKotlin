...
sealed class BottomNavigationViewBindingAdapter {

  companion object {
    private const val NO_ID = -1

    @BindingAdapter(value = ["selectedItem"])
    @JvmStatic fun viewNavigationBindSelectedItem(viewNavigation: BottomNavigationView, menuItemRes: Int = NO_ID) {
    ...
    @JvmStatic fun viewNavigationBindSelectedItem(viewNavigation: BottomNavigationView): Int {
      val menu = viewNavigation.menu
      (0..menu.size())
          .map { menu.getItem(it) }
    ...
    @JvmStatic fun viewNavigationBindOnNavigationSelected(viewNavigation: BottomNavigationView, navigationSelected: OnNavigationSelected?, selectedItemAttr: InverseBindingListener?) {
